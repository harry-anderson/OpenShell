...
<Tip>
To sign git commits with the host SSH agent (including on Kubernetes), pass `--forward-agent`. See [SSH agent forwarding](/sandboxes/ssh-agent-forwarding). This is opt-in: the sandbox becomes a signing oracle for your host key.

```shell
openshell sandbox create --forward-agent -- claude
```
